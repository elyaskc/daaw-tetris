...
import { ROWS, COLS, SHAPES, COLORS } from '../../constants'; // Asegúrate de que la ruta a constants sea correcta
import { sendAttack, listenForAttacks } from '../../persistence/database';
...
const TetrisBoard = ({
  ...
  const [nextPieceInternal, setNextPieceInternal] = useState(randomPiece());
  const [piece, setPiece] = useState(randomPiece());

  const speed = 1000 * Math.pow(0.9, Math.max(0, level - 1));

  useEffect(() => {
    setNextPiece(nextPieceInternal);
  }, []);
  ...
  const handleLineClear = (cleared) => {
    if (cleared > 0) {
      setLines((l) => l + cleared);
      sendAttack(cleared, player);
    }
  };

  const spawn = () => {
    const pieceToSpawn = nextPieceInternal;
    
    const newPiece = randomPiece();
    setNextPieceInternal(newPiece);
    setNextPiece(newPiece);

    if (collide(board, pieceToSpawn, 0, 0)) setGameOver(true);
    else setPiece(pieceToSpawn);
  };
  ...
  const addGarbageLines = (amount) => {
    setBoard((prevBoard) => {
        const newBoard = prevBoard.map(row => [...row]);
        newBoard.splice(0, amount);
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenForAttacks(player, (amount) => {
        if (!paused && !gameOver) addGarbageLines(amount);
    });
    return () => unsubscribe();
